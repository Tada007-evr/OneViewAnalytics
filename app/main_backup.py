...
def current_student():
    ...
    # Try the Streamlit session first
    user = st.session_state.get("user")

    if user:
        return user

    # Otherwise restore the Supabase session
...
app()
    ...
    if page == "Overview":
        ...
        st.write(f"Signed in")
        page = st.radio("Navigation", ["Overview", "Record Practice", "History", "Insights"])
        if st.button("Sign out"):
            sb.auth.sign_out()
            st.session_state.pop("user", None)
            st.rerun()
